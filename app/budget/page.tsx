...
export default function BudgetPage() {
  ...
  async function handleSave() {
    ...
    for (const [cat, raw] of Object.entries(budgets)) {
      ...
      if (!isNaN(val) && val > 0) payload[cat] = val
    }
    try {
      const res = await fetch('/api/user', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ category_budgets: payload }),
      })
  ...
  const totalBudget = Object.values(budgets).reduce((sum, v) => {
          ...
          <div className="max-w-lg mx-auto space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-stone-400 mt-1">
                Set a spending limit for each category. Your dashboard will track progress.
              </p>
            </div>
              ...
              <div className="space-y-2">
                {BUDGET_CATEGORIES.map((cat) => {
                  ...
                  return (
                    <div
                      key={cat}
                      className="rounded-2xl bg-white border border-stone-100 px-4 py-3"
                    >
            ...
            {!loading && (
              <div className="flex items-center justify-between pt-2">
                <p className="text-sm text-stone-500">
                  {totalBudget > 0 ? `Total: ${fmt(totalBudget)} / month` : 'No budgets set yet'}
                </p>
                <button
                  onClick={handleSave}
                  disabled={saving}
