  ...
  async function handleEstimate() {
    ...
    try {
      ...
      setBudgets((prev) => {
        const next = { ...prev }
        for (const cat of BUDGET_CATEGORIES) {
          if (estimates[cat] != null) {
            next[cat] = String(estimates[cat])
          }
        }
        return next
      })
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-stone-400 mt-1">
                  Set a limit per category. Your dashboard tracks overall and per-category progress.
                </p>
              </div>
              <button
            ...
            {!loading && (
              <div className="flex items-center justify-between pt-2">
                <p className="text-sm text-stone-500">
                  {categoryTotal > 0 ? `Total: ${fmt(categoryTotal)} / month` : 'No budgets set yet'}
                </p>
                <button
                  onClick={handleSave}
                  disabled={saving}
